...
Người dùng nhập “Xem nhân vật hiện tại và đề xuất việc farm”, chọn target/ảnh. Hệ thống trả điều nhìn thấy kèm vùng ảnh, điều chưa chắc và phương án. Sau khi direct-host workflow đạt gate, người dùng giao gói thu tài nguyên; local tự quan sát–hành động–xác nhận trên cùng máy, dừng khi lệch phạm vi. Có hỏi trạng thái, giải thích, pause/cancel/resume. CLI là giao diện đầu; chat localhost có ảnh và lịch sử là bước tiếp theo.

## Yêu cầu chức năng và nghiệm thu

| ID | Yêu cầu | Bằng chứng |
|---|---|---|
| F01 | Task có intent, target/session ID, allowed workflows/actions, expiry và ngân sách | Không đủ binding/quyền/ảnh mới thì không dispatch |
| F02 | PNG/JPEG thật trên host hoặc ảnh đã nhập; hash, kích thước, thời điểm capture nếu biết và thời điểm nhập riêng | Ảnh vừa nhập không tự thành ảnh live; timestamp không rõ ghi null |
...
| F06 | Computer Use trực tiếp trên Windows host được kiểm chứng | Runtime evidence zero input ngoài allowlist/focus ngoài target; không desktop fallback |
...
CLI/chat → task contract → capture/image → OCR + vision → scene → local model → typed intent → policy → direct host action → fresh observation → verifier → report/checkpoint.
...
## Windows host trực tiếp và không chiếm input ngoài allowlist

Sản phẩm chạy trực tiếp trên một máy Windows với một user; Docker, VM và hypervisor không thuộc sản phẩm. Phải xác minh render game, capture thật, target/frame binding, foreground ổn định và input chỉ đi qua allowlist Win32 trong action window. Không dùng PostMessage, cửa sổ ẩn, desktop fallback hoặc giả định người dùng có thể tiếp tục điều khiển foreground cùng lúc.

Máy: Windows10 Pro, CPU/RAM host, không yêu cầu hypervisor hay GPU passthrough. Observer và action path phải fail-closed theo HWND/PID/geometry, không fallback sang desktop mơ hồ.
...
- **R2:** direct-host trace ngắn trước action; operator quiescent, zero focus/input ngoài allowlist, target/frame binding đúng; thử stale-frame/cancel. Có runtime trace, không chỉ grep thư viện.
...
| P4 | do_duong → tho_dung | Direct Windows host | Target/input-isolation evidence → guarded action bridge; R2 |
...
Ảnh ROK gán nhãn và live workflow acceptance còn thiếu. Không cài hypervisor/VM/Docker, không reboot hoặc mua hardware cho sản phẩm này. Robotics/model training/global keyboard logging và tự đổi tài khoản/mua vật phẩm không thuộc đợt đầu. Legacy source không được import để thử nhanh.
